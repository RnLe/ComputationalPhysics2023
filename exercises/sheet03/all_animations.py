...
files_r = ["b)r_n4.dat", "b)r_n8.dat", "b)r_n16.dat"]

# List of set data files
files_set = ["b)set_n4.dat", "b)set_n8.dat", "b)set_n16.dat"]
...
def update(frame):
    for scatter, data, text, single_data_set, (line1, line2, line3) in zip(scatters, data_r, texts, data_set, lines):
        ...
        time, T, Ekin, Epot, vx_, vy_ = single_data_set
        text.set_text(f't={time[frame]:.2f} T={T[frame]:.2f} Ekin={Ekin[frame]:.2f} Epot={Epot[frame]:.2f}')
...
ani.save('animation_b.mp4', writer='ffmpeg', fps=25, progress_callback=update_progress)
pbar.close()
...
ani.save('animation_c.mp4', writer='ffmpeg', fps=25, progress_callback=update_progress)
pbar.close()

# ==========================================================================

# List of position data files
files_r = ["d)r0.01.dat", "d)r1.dat", "d)r100.dat"]

# List of set data files
files_set = ["d)set0.01.dat", "d)set1.dat", "d)set100.dat"]
...
ani.save('animation_d.mp4', writer='ffmpeg', fps=25, progress_callback=update_progress)
pbar.close()